...
class FeaturePipeline:
    ...
    def _build_feature_names(self):
        """Build list of feature names for all groups."""
        names = []
        ...
        for group in self.groups:
            if group == 'lexical':
                ...
                # an expanded per-word frequency vector depending on config.
                if self.use_per_word_fw:
                    names.extend(function_words.get_names_per_word())
                else:
                ...
                # POS features are generated by pos_features and must agree with
                # the extractor output shape.
                names.extend(pos_features.NAMES)

            elif group == 'ngram':
                # N-gram LM features: 3 per n (log_prob, perplexity, entropy)
                if hasattr(self, '_ngram_extractor') and hasattr(self._ngram_extractor, 'names'):
                    names.extend(self._ngram_extractor.names)
                else:
    ...
    def fit(self, sentences):
        """Fit all trainable components (n-gram models, etc.)."""
    ...
    def describe(self):
        ...
        # Add to src/features/pipeline.py inside FeaturePipeline class
    ...
    def fit(self, sentences):
        ...
        # Fit any other trainable components
        if hasattr(self, '_char_ngram_extractor') and self._char_ngram_extractor:
            if hasattr(self._char_ngram_extractor, 'fit'):
                self._char_ngram_extractor.fit(sentences)
